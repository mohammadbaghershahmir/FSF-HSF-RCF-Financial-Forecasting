...
class SFTDataLoader(object):
    ...
    """
            if data_point["input"]
            else f"""Below is an instruction that describes a task. Write a response that appropriately completes the request.
    ...
    def load_data(self):
        print("Sample data:", self.data["train"][:2])  # چاپ نمونه داده‌ها
        if self.VAL_SET_SIZE > 0:
            ...
            train_data = self.data["train"].shuffle().map(self.generate_and_tokenize_prompt)
            val_data = None
